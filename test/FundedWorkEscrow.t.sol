...
contract FundedWorkEscrowTest is Test {
    ...
    function testPauseBlocksCreateJob() public {
        ...
        escrow.createJob(10, 1e6, uint64(block.timestamp + 7 days), "ipfs://tasks", false);
    }
    ...
    function testWhitelistGatedJob() public {
        vm.prank(client);
        uint256 jobId = escrow.createJob(10, 1e6, uint64(block.timestamp + 7 days), "ipfs://tasks", true);
        
        address unauthorizedWorker = address(0xBAD);
        vm.prank(unauthorizedWorker);
        vm.expectRevert("not whitelisted");
        escrow.submitWork(jobId, 4, proof);
        
        address[] memory allowed = new address[](1);
        allowed[0] = worker;
        vm.prank(client);
        escrow.whitelistWorkers(jobId, allowed);
        
        vm.prank(worker);
        escrow.submitWork(jobId, 4, proof); // Should succeed
    }
}