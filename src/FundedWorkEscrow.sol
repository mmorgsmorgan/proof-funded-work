...
contract FundedWorkEscrow {
    ...
    function createJob(uint64 totalTasks, uint128 rewardPerTask, uint64 deadline, string calldata metadataURI) external nonReentrant returns (uint256 jobId) {
    ...
    function submitWork(uint256 jobId, uint64 submittedTasks, bytes32 proofHash) external returns (uint256 submissionId) {
    ...
    function verifyWork(uint256 submissionId, uint64 approvedTasks) external nonReentrant {
        ...
        require(msg.sender == job.client || verifiers[msg.sender], "not job reviewer");
