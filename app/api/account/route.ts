...
export async function GET(request: Request) {
  ...
  try {
    ...
    const account = await findAccountByPrivyId(identity.privyUserId);
    if (!account) {
      done(404);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const identity = await verifyPrivyRequest(request);
    const account = await createAccount({ ...identity, role: body.role });
    const responseBody = JSON.stringify({ account });
    const status = 201;

    if (idempotencyKey) {
      await saveIdempotency(idempotencyKey, status, responseBody);
    }
