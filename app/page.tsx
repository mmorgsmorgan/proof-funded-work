...
function jobCopy(uri: string) { return uri || 'Verified task batch'; }
...
function Onboarding({ busy, accountError, onChoose }: { busy: boolean; accountError: string; onChoose: (role: Role) => void }) {
  return <div className="app-shell"><header className="app-topbar container"><a className="site-name" href="/">Q&apos;IT</a><span className="app-network">Arc testnet / onboarding</span><span className="app-status">Wallet created</span></header><main className="onboarding container"><span className="label">Your account is ready</span><h1>What will you do<br /><em>with funded work?</em></h1><p>Choose one role. You can browse the market and use your embedded Arc wallet immediately.</p><div className="role-cards"><button disabled={busy} onClick={() => onChoose('worker')}><span>01</span><strong>Worker</strong><small>Complete verified tasks and claim approved USDC.</small><b>Enter as worker ↗</b></button><button disabled={busy} onClick={() => onChoose('task_giver')}><span>02</span><strong>Task giver</strong><small>Create a job, fund its escrow, and review submitted work.</small><b>Enter as task giver ↗</b></button></div>{accountError && <p className="workspace-error">{accountError}</p>}</main></div>;
}
...
function Workspace({ account, wallet }: { account: Account; wallet?: `0x${string}` }) {
  ...
  async function createJob() {
    ...
    setBusy(true); setMessage('Approve USDC, then confirm the funded job…');
    try {
      if (!embeddedWallet || !wallet) throw new Error('Your embedded wallet is not ready yet.');
